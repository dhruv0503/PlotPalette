...
module.exports.getMovie = async (req, res, next) => {
    try {
        ...
        if (querySnapshot.docs.length > 0) {
            ...
            if (userRef !== null) {
                ...
                if (!movies || !movieObj) {
                    ...
                    if (movieObj.data().watched) {
                        ...
                        res.send({ watchedByUser: false, watchLaterByUser: uSMD.watchLater, id : movieData.id, loggedIn: true, ...movieData.data() })
                        return;
                    }
                }
            }
            ...
            if (userRef !== null) {
                const user = await utilityFunctions.getUser(userRef);
                const obj = { watched: false, tmdbId, movieId: newMovieData.id, "title": newMovieData.data().title, rating: 0, favourite: false, watchLater: false, poster: newMovieData.data().poster_path }
                await addDoc(collection(doc(User, user.id), 'movies'), obj);
                res.send({ watchedByUser: false, loggedIn: true, id: newMovieData.id, ...newMovieData.data() })
                return;
            }
...
module.exports.watchLater = async (req, res, next) => {
    const user = auth.currentUser;
    const { tmdbId, watchLater } = req.query;
