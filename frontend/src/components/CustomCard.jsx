...
//             </div>
        ...
        <div className="max-w-sm bg-custom-50 p-6 overflow-hidden shadow-lg relative">
            ...
            <div className="relative">
                ...
                <div className="absolute inset-0 flex flex-col justify-center items-center opacity-0 hover:opacity-95 transition duration-300">
                    <div className="font-bold text-grey-200 text-xl mb-2 bg-white rounded-lg p-2">{title}</div>
                    <div className="text-gray-200 text-sm mb-2">{`Release Date: ${release_date}`}</div>
                    <div className="flex">
                        {matchid.map(genre => (
                            <p className="text-gray-200 text-sm inline-block m-1" key={genre.id}>{genre.name}</p>
                        ))}
                    </div>
                    <div className='mt-4'>
                        <Link to={`/movies/${title}`} className='block bg-custom-10 border border-black p-2 rounded-b-lg text-center text-custom-50 hover:bg-custom-30 hover:text-white transition duration-300'>
