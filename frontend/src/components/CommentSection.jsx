...
const CommentSection = ({ props, watched, watch }) => {
    ...
    const MovieDetails = async () => {
      try {
        const MovDetails = await axios.get(
          `${process.env.REACT_APP_BACKEND_URL}api/movies/reviews?tmdbId=${props}`
        );
  ...
  const handlePost = async (e) => {
    ...
    try {
      await axios.post(
        `${process.env.REACT_APP_BACKEND_URL}api/reviews?tmdbId=${props}`,
        { reviewText }
      );
      setreviewText("");
  ...
  const handleUpdateReview = async (e) => {
    ...
    try {
      await axios.put(
        `${process.env.REACT_APP_BACKEND_URL}api/reviews?tmdbId=${props}&reviewId=${reviewid.id}`,
        { reviewText }
      );
      setreviewText("");
      window.location.reload();
  ...
  const handleDelete = async ({ rid }) => {
    try {
      await axios.delete(`${process.env.REACT_APP_BACKEND_URL}api/reviews?tmdbId=${props}&reviewId=${rid}`);
      window.location.reload();
    } catch (error) {
  ...
  const handleUpvote = async ({ id }) => {
    try {
      await axios.put( `${process.env.REACT_APP_BACKEND_URL}api/reviews/upvote?reviewId=${id}`);
  ...
  const handleDownvote = async ({ id }) => {
    try {
      await axios.put(`${process.env.REACT_APP_REACT_APP_BACKEND_URL}api/reviews/downvote?reviewId=${id}`);
      // Handle user details (e.g., display username)
      window.location.reload();
    } catch (error) {
      console.error("Error fetching user:", error.message);
        ...
        {localStorage.getItem("uid") ? (
          <form method="get">
                ...
                <div class="flex flex-row space-y-2 items-center justify-center h-full py-4 bg-gray-800 rounded-xl space-x-10">
                  <div class="w-2/3">
                    ...
                    <p class="w-full pb-8 text-sm tracking-wide leading-tight text-white">
                      You can only comment if you have watched the movie
                    </p>
                    <div class="rounded w-1/3">
                      <div class="opacity-95 border rounded-lg border-white px-4">
                        <button
                          onClick={watch}
                          class=" flex  m-auto gap-3 items-center text-sm font-medium leading-normal  text-white py-2"
                        >
            ...
            {watched && (
              <button
                onClick={handlePost}
                className="mt-2 px-4 py-2 bg-custom-50 text-white rounded hover:bg-black"
              >
                Post Comment
              </button>
            )}
              ...
              <div className="flex m-2 ">
                ...
                <div>
                  <button
                    ...
                    onClick={() =>
                      handleUser(navigate(`/account/${movie?.reviewer}`))
                    }
                  >
                    {movie?.userName}
                  </button>
                  ...
                  <div className="flex   items-center gap-3 ">
                    {movie?.vote == "upvote" ? (
                      ...
                      <ThickArrowUpIcon
                        className="text-custom-30  "
                        height={24}
                        width={24}
                        onClick={() => handleUpvote({ id: movie.reviewId })}
                      />
                    )}
                    ...
                    {movie.vote == "downvote" ? (
                      ...
                      <ThickArrowDownIcon
                        height={24}
                        width={24}
                        onClick={() => handleDownvote({ id: movie.reviewId })}
                      />
                    )}
                    ...
                    {movie?.owner && (
                      <>
                        <UpdateIcon
                          height={24}
                          width={24}
                          onClick={() => textfield({ id: movie.reviewId })}
                        />
                        <Cross2Icon
                          height={24}
                          width={24}
                          onClick={() => handleDelete({ rid: movie.reviewId })}
                        />
                      </>
                    )}
                  </div>
